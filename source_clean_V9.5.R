...
'plots all species in a level of sim to species focus in the first level round'
...
# sp_focus is a species or a vector of species or a data frame with SCINAME column
    ...
    for (n in 1:n_levels){
        ...
        list_spp[[level]] = sp_df      # if wanna individual level lists turn it on - it makes the final object a lit bit too confuse but makes things easier to plot
...
#___call_function_sp_levels_sim _________________________________________________________________________________________
        ...
        # if(any(threshold_results$status == 'CLOSED')){
        #     # to have areas, intersections, etc habilitate these two lines
        #     if(areas) {threshold_results = left_join(threshold_results, sim_df, by = c('sp_focus' = 'species1', 'SCINAME' = 'species2')) %>%
        #         filter(sp_focus != SCINAME) %>% select(sp_focus, SCINAME, dist_sim, min_sim, level, n_lev, status, everything())
        #     } else {threshold_results = left_join(threshold_results, sim_df, by = c('sp_focus' = 'species1', 'SCINAME' = 'species2')) %>%
        #         filter(sp_focus != SCINAME) %>% select(species1 = sp_focus, species2 = SCINAME, dist_sim, congruence = min_sim, level, status)}
        #     
        #     # if(areas) {threshold_tab_min_partial = threshold_results %>% filter(status == 'CLOSED') %>% # need to check these 'areas = T' regions
        #     #     select(species1 = sp_focus, species2 = SCINAME, dist_sim, min_sim, level, n_lev, everything()) %>% 
        #     #     group_by(species1, species2, dist_sim, level, status) %>% 
        #     #     summarise(n=n(), min=range(min_sim)[1], max=range(min_sim)[2]) %>% 
        #     #     select(species1, species2, n, dist_sim, level, status,  max, min, everything()) %>% as.data.frame()
        #     # } else {
        #     #     threshold_tab_min_partial = threshold_results %>% filter(status == 'CLOSED') %>% 
        #     #     select(species1 = sp_focus, species2 = SCINAME, dist_sim, congruence = min_sim, level, status ) %>% 
        #     #     group_by(species2,min_sim) %>% filter(level == min(level)) %>% as.data.frame
        #     # }
        #         
        #     if(tab_min){
        #         threshold_tab_min_partial = threshold_results %>% group_by(species1,species2) %>% filter(congruence == max(congruence)) %>% as.data.frame
        #         threshold_tab_min = rbind(threshold_tab_min, threshold_tab_min_partial) %>% mutate(dist_sim = round(dist_sim,2))
        #     # threshold_tab_min = threshold_tab_min %>% unique() # changed to unique()
        #     }
        # }
    }  # for loop ends here
...
'to calculate internal or external status of sp_focus compared to  another polygons in a df$SCINAME:'
'description'
...
co_occurrence_patterns = function (test_mode = FALSE, D_df=D_df,   # similarity matrix (vide dist_sim function)
...
'calcula inter union rate after incorporating each species sorted by sim'
...
# A helper function that erases all of y from x: (from help st_difference)
st_erase = function(x, y) st_difference(x, st_union(st_combine(y)))

# read input integer
readinteger <- function(){ 
    n <- readline(prompt="Numeric Password, please: ")
    return(ifelse(n==0,'cu, acertou!', paste(as.integer(n),' very good memory!')))
}
...
test = ggpatt(sp_focus=spp, plot_ae = T)
test
}

# https://stackoverflow.com/questions/25636897/get-map-with-specified-boundary-coordinates
foo <- ggmap(ggmap_bbox(sa_ggmap_z4)) + ggtitle('title') + theme(plot.title = element_text(size = 20, face = "bold"))+
...
cg_chorotype_plot = function(sp_focus = sp_focus, congruence = 1, level = 1, bbox = FALSE, sp1_sp2 = sp1_sp2, new_plot = FALSE, map_species,...){
    ...
    'threshold levels - all closed systems from sp_focus'
...
# title with 2 colors template obs expression do not read objects
# plot(x, y)
...
# http://r.789695.n4.nabble.com/How-do-I-change-which-R-Graphics-Device-is-active-td897225.html
# > dev.new();ggpatt = dev.cur() 
# > dev.new();xyPlot = dev.cur() 
# > dev.set(ggpatt); plot something... 
# X11cairo 
# 2 
